...
def csv_to_cypher(csv_path):
    ...
    for i, row in enumerate(rows):
        ...
        for idx, value in enumerate(row[1:]):
            ...
            if i > idx:
                if value[0] == '1':
                    if source == 'power_cable' or source == 'Power_Button' or source == 'motors' or (source=="power_cable" and (target=="battery_charging_port" or target=="controller")): 
                        relationships.append(f'CREATE ({source})-[:energy]->({target})')
                        print('source, energy', source)
                    else:
